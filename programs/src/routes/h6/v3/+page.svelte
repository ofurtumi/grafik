<script lang="ts">
  ...
  import { normalCube } from "$lib/Helpers";
  ...
  void main() {
    ...
    R = 2.0 * max(dot(L,N), 0.0) * N - L; // 2.0*max(dot(L, N), 0.0)*N - L;
  ...
  let phongLoc: WebGLUniformLocation | null = null;
  ...
  const buffer = (
    ...
    const [points, normals] = normalCube(0.5);
    ...
    phongLoc = gl.getUniformLocation(prog, "phong");
  };

  const render = (gl: WebGLRenderingContext) => {
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);

    gl.uniform1i(phongLoc, phong ? 1 : 0);
    gl.uniform4fv(lightPositionLoc, flatten(lightPosition));
    ...
    mv_c = mult(mv_c, scalem(vec3(0.1, 1.0, 0.1))) as matrix;
    gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(mv_c));
    ...
    // top
    mv_c = mult(mv, translate(vec3(0.0, 0.3, 0.0))) as matrix;
    mv_c = mult(mv_c, scalem(vec3(1.0, 0.2, 0.8))) as matrix;
    gl.uniformMatrix4fv(modelViewMatrixLoc, false, flatten(mv_c));
    gl.drawArrays(gl.TRIANGLES, 0, num_points);
...
<p>
  Notið hakið hér fyrir neðan eða smellið á "p" til að skipta á milli Phong og
  Blinn Phong lýsingar
</p>

<input id="phong" type="checkbox" bind:checked={phong} />
<label for="phong">{phong ? "Phong" : "Blinn Phong"}</label>
