<script lang="ts">
  ...
  let thresholdLoc: WebGLUniformLocation | null = null;

  let size = 0;
  let step = 1;

  let current_quality = 4;
  ...
  const redo_tepot = (quality: number, gl: WebGLRenderingContext) => {
    size += step;
    const scale = Math.abs(Math.sin(radians(size)));
  ...
  const buffer = (
    ...
    thresholdLoc = gl.getUniformLocation(prog, "threshold");
  };
  ...
  const keydown = (e: KeyboardEvent) => {
    switch (e.key) {
      case "ArrowLeft":
        threshold = Math.max((threshold * 10 - 1) / 10, 1);
        break;
      case "ArrowRight":
        threshold = Math.min((threshold * 10 + 1) / 10, 3);
        break;
      case "ArrowDown":
        step -= 1;
        break;
...
<p>Hægri og vinstri örvar til að auka / minnka þröskuldin fyrir ljósið</p>
<p>Upp og niður örvar til að auka hraða á skölun</p>

<p>Þröskuldur: {threshold}</p>
<p>Hraði: {step}</p>
